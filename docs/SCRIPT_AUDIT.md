...
Never does:
- Does not delete organized media libraries
...
- Keeps placeholder directories (`state/`, `logs/`, `photos/`, `videos/`) for receiver readiness
- Optional: removes local photo/video libraries when `-IncludeMedia` is passed (legacy `-IncludePhotos` remains supported)

Creates/updates:
- Ensures placeholder directories exist after cleanup

Never does:
- Does not remove organized media libraries unless `-IncludeMedia` (or legacy `-IncludePhotos`) is requested
...
- Unsupported temporary files are moved to state/quarantine.
- Organized libraries under photos/YYYY/MM/DD and videos/YYYY/MM/DD are preserved unless explicit delete options are enabled.
